...
// The OpenAI chat-completions request/response shape (and its SSE streaming
// format) has become a de-facto standard — Groq, Mistral, DeepSeek, xAI, and
// OpenRouter (among others) all speak it, differing only in base URL and
// which of the optional fields they actually honor. One implementation
// parameterized by base URL covers all of them instead of near-duplicate
// per-vendor clients.
export function createOpenAiCompatibleChat(baseUrl: string, providerLabel: string): ChatFn {
    const chat: ChatFn = async (apiKey, model, messages, options, onToken, signal, tools) => {
        ...
        try {
            ...
            throw describeNetworkError(providerLabel, err);
        }

        if (!res.ok || !res.body) {
            throw new Error(await describeHttpError(res, providerLabel));
        }

        // Tool-call arguments arrive as fragmented JSON string deltas, indexed
        // by position in the tool_calls array, across many SSE events — only
        // valid to parse once the stream signals finish_reason "tool_calls".
